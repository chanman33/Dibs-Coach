...
export default function LOProfilePage() {
  ...
  const { 
    ...
    setValue,
    formState: { errors }
  } = useForm<LOProfileFormData>()

  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true)
        const profileResponse = await fetch('/api/user/loan-officer')
...
//TO DO: Create API endpoint for updating loan officer profile

  const onSubmit = async (data: LOProfileFormData) => {
    setLoading(true)
    try {
      console.log('[PROFILE_UPDATE] Sending data:', data)
      const response = await fetch('/api/user/loan-officer', {
        method: 'PUT',
  ...
  return (
    <div className="container max-w-2xl py-8">
      <Card>
        <CardHeader>
          <h2 className="text-2xl font-bold">Loan Officer Profile</h2>
        </CardHeader>
        <CardContent>
          {isLoading ? (
