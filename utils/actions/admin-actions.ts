...
export const updateUserCapability = withServerAction<{ success: boolean }>(
  async (params: { 
    userUlid: string
    capability: keyof typeof USER_CAPABILITIES
    action: 'add' | 'remove'
  }, { systemRole }) => {
    try {
      if (systemRole !== SYSTEM_ROLES.SYSTEM_OWNER) {
        return {
          ...
          error: {
            code: 'FORBIDDEN',
            message: 'Only system owners can update user capabilities'
          }
        }
      }
      ...
      if (fetchError) {
        console.error('[UPDATE_USER_CAPABILITY_ERROR]', fetchError)
        throw fetchError
      }

      // Update capabilities
      let capabilities = Array.isArray(userData.capabilities) ? [...userData.capabilities] : []
      
      if (params.action === 'add' && !capabilities.includes(params.capability)) {
        capabilities.push(params.capability)
      } else if (params.action === 'remove') {
        capabilities = capabilities.filter(c => c !== params.capability)
      }

      // Determine boolean flags from capabilities
      const isCoach = capabilities.includes(USER_CAPABILITIES.COACH)
      const isMentee = capabilities.includes(USER_CAPABILITIES.MENTEE)
      ...
      if (updateError) {
        console.error('[UPDATE_USER_CAPABILITY_ERROR]', updateError)
        throw updateError
      }
      ...
      const { error: logError } = await supabase
        ...
        .insert({
          ulid: '', // Will be generated by DB
          type: 'USER_ACTION',
          title: 'User Capability Updated',
          description: `User ${params.action === 'add' ? 'granted' : 'removed'} ${params.capability} capability`,
          severity: 'info',
          createdAt: new Date().toISOString(),
          updatedAt: new Date().toISOString()
        })
      ...
      if (logError) {
        console.error('[LOG_USER_CAPABILITY_ERROR]', logError)
        // Don't throw here as the main action succeeded
      }
      ...
      console.error('[UPDATE_USER_CAPABILITY_ERROR]', error)
      return {
        data: null,
        error: {
          code: 'INTERNAL_ERROR',
          message: 'Failed to update user capability',
          details: error instanceof Error ? { message: error.message } : undefined
        }
      }
    }
