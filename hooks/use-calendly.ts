...
export function useEventInvitees() {
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<Error | null>(null)
  const [invitees, setInvitees] = useState<CalendlyInvitee[]>([])

  const fetchInvitees = useCallback(async (params: {
    eventUuid: string
    count?: number
    pageToken?: string
  }) => {
    try {
      ...
      const response = await fetch(`/api/calendly/scheduled-events/${params.eventUuid}/invitees?${searchParams}`)
      if (!response.ok) {
  ...
  const markAsNoShow = useCallback(async (inviteeUri: string) => {
    try {
      setLoading(true)
      const response = await fetch('/api/calendly/invitees/no-show', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const undoNoShow = useCallback(async (inviteeUuid: string) => {
    try {
      setLoading(true)
      const response = await fetch(`/api/calendly/invitees/${inviteeUuid}/no-show`, {
        method: 'DELETE',
      })
      ...
      toast.success('Undo no-show successful')
    } catch (err) {
